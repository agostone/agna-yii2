...
class Mailer extends Component implements MailerInterface, ViewContextInterface
{
     ...
     */
    public $messageClass = 'Agna\Yii2\Mail\Zend\Message';
    ...
    public function setTransport($transport)
    {
        ...
        if (is_array($transport)) {
            ...
            // Because zend transport classes using special options classes, that's all folks.
            if (isset($transport['options']) && is_array($transport['options'])) {
                $transportOptions = Yii::createObject($transport['options']);
                unset($transport['options']);
            }

            $transport = Yii::createObject($transport, [$transportOptions]);
        }

        if (!$transport instanceof \Zend\Mail\Transport\TransportInterface) {
            throw new InvalidParamException('transport', $value, '\Zend\Mail\Transport\TransportInterface');
        }
     ...
     * It will call [[sendMessage()]] to send the email to its recipient(s).
    ...
    public function send($message)
    {
        ...
        Yii::info('Sending email "' . $message->getSubject() . '" to "' . $addresses . '"', __METHOD__);
